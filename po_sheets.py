...
PO_SEARCH_URL = f"https://{LOGIN_ID_1_ENC}:{LOGIN_PASS_1_ENC}@{DOMAIN}/po-heads"
GOOGLE_CREDENTIALS = os.environ["GOOGLE_CREDENTIALS"]
SPREADSHEET_ID = os.environ["SPREADSHEET_ID"]
# POデータの書き込み先（専用スプレッドシート）。未設定なら従来のSPREADSHEET_IDを使う。
PO_SO_SPREADSHEET_ID = os.environ.get("PO_SO_SPREADSHEET_ID") or SPREADSHEET_ID

KEY_COLUMNS = ["PO#", "Product ID"]
DATE_COLUMN = "PO Created Date"


def _fetch_po_range(page, context, start_date, end_date):
    """指定期間のCSVを1回ダウンロードし、(status_code, 行リスト) を返す。

    行リストは [ヘッダー行, データ行...]。失敗時(200以外)は (status, None)。
    """
    page.goto(PO_SEARCH_URL, wait_until="networkidle")
    ...
    download_link = page.locator('a:has-text("Download PO lines")').first
    href = download_link.get_attribute("href")
    if not href:
        raise Exception("Download PO lines のリンクが見つかりませんでした")
...
def _fetch_po_range_recursive(page, context, start, end):
    ...
    for attempt in range(1, 5):  # 最大4回
        try:
            status, rows = _fetch_po_range(page, context, start_str, end_str)
            break
    ...
    left_header, left_rows = _fetch_po_range_recursive(page, context, start, mid)
    right_header, right_rows = _fetch_po_range_recursive(page, context, mid + timedelta(days=1), end)
    return (left_header or right_header), (left_rows + right_rows)


def download_po_csv(backfill_months=None):
    """PO検索画面からCSVをダウンロードする
    ...
    with sync_playwright() as p:
        ...
        header, data_rows = _fetch_po_range_recursive(page, context, start, today)
    ...
    # バックフィルで期間分割した場合、境界での重複を念のため除去（PO# + Product ID）
    if header:
        try:
            po_i = header.index("PO#")
            pid_i = header.index("Product ID")
            seen = set()
            deduped = []
            for row in data_rows:
                if len(row) <= max(po_i, pid_i):
                    deduped.append(row)
                    continue
                key = (row[po_i], row[pid_i])
                if key in seen:
                    continue
                seen.add(key)
                deduped.append(row)
...
def parse_po_date(date_str):
    """PO Created Date の文字列をdatetimeに変換する"""
    if not date_str:
        return None
    try:
...
def update_po_sheet(csv_path, backfill=False):
    """POタブのデータを更新する（重複スキップ・差分更新・6ヶ月超削除）

    backfill=True の場合は、既存データを全消去してCSVの内容で丸ごと入れ替える（初回一括取り込み用）。
    """
    print("Googleスプレッドシート（POタブ）を更新しています...")
    ...
    try:
        ...
        print("「PO」タブが無いため新規作成します")
        worksheet = spreadsheet.add_worksheet(title="PO", rows=100, cols=20)
    ...
    if backfill:
        ...
        update_status("PO")
        update_status("PO", spreadsheet_id=PO_SO_SPREADSHEET_ID)
        return

    try:
        po_idx_new = new_headers.index("PO#")
        pid_idx_new = new_headers.index("Product ID")
        date_idx_new = new_headers.index(DATE_COLUMN)
    ...
    try:
        po_idx_ex = existing_headers.index("PO#")
        pid_idx_ex = existing_headers.index("Product ID")
    ...
    for offset, row in enumerate(existing_rows):
        ...
        if len(row) > max(po_idx_ex, pid_idx_ex):
            key_to_rownum[(row[po_idx_ex], row[pid_idx_ex])] = rownum
        existing_by_rownum[rownum] = row
        d = parse_po_date(row[date_idx_ex]) if len(row) > date_idx_ex else None
        if d is not None and d < six_months_ago:
            old_rownums.append(rownum)

    # 取得分を「新規追加」「変更あり既存」に振り分け
    to_append = []
    ...
    for new_row in new_data:
        if not any(new_row):
            continue
        if len(new_row) <= max(po_idx_new, pid_idx_new):
            continue
        key = (new_row[po_idx_new], new_row[pid_idx_new])
        if key in seen_new:
...
if __name__ == "__main__":
    print("=== Daily PO CSV → Google Sheets (POタブ) ===")
    ...
    csv_path = download_po_csv(backfill_months)
    update_po_sheet(csv_path, backfill=bool(backfill_months))
    print("=== 完了 ===")
