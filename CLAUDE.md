...
## 出品データ同期ツール（rakuten_listing_sync.py）

上記の赤字チェックとは別に、**楽天RMS自体に出品している商品の商品名・価格・在庫状況**を毎日取得し、専用スプレッドシートへ「最新スナップショット」として記録するツール。Yahoo!ショッピング（ストアクリエイターPro API）も対象にする予定だが、まずは楽天のみ実装（Phase 1）。

### 背景（なぜ作ったか）

Purchaserが「Amazonで購入不可」と判断した商品について、その商品が楽天・Yahooにも出品されている場合はそちらもCloseすべきだが、過去に売れたことがない・社内システムに未登録の商品はPurchaser側で出品の存在に気づけず、Closeが漏れてAlt Offerにつながるケースがある。将来的には、Purchaser側の「購入不可」報告（`shopping_report_process.py`のNot Bought/Close処理）とこの出品データを突き合わせ、対応する楽天・Yahoo出品を自動的に洗い出す仕組みへ拡張する構想（未着手・別途計画予定）。今回のPhase 1では、その土台となる「出品データの日次スナップショット」のみを実装する。

### 対象API・保存先

| 項目 | 内容 |
|---|---|
| データソース | 楽天RMS API 商品検索API `items.search`（`GET https://api.rms.rakuten.co.jp/es/2.0/items/search`） |
| 対象店舗 | **楽天2店舗**（`RAKUTEN_SHOP_NAME_1/2` + 店舗ごとのserviceSecret/licenseKey）。店舗を増やす場合は`rakuten_listing_sync.py`の`STORES`リストと環境変数を追加する |
| 認証方式 | `Authorization: ESA <Base64(serviceSecret:licenseKey)>`（店舗ごとに個別のキーペア）。ライセンスキーは3か月ごとに手動更新が必要 |
| ページング | `hits`（最大100、未指定時デフォルト10のため必ず指定）+ `cursorMark`。リクエストの`cursorMark`とレスポンスの`nextCursorMark`が一致したら終了 |
| 保存先 | 出品データ同期専用スプレッドシート（`RAKUTEN_LISTING_SPREADSHEET_ID`、赤字チェック用の`RAKUTEN_SPREADSHEET_ID`とは別ファイル） |
| シート/タブ名 | 「楽天_出品データ」（2店舗分を1つのタブに集約） |
| 実行頻度 | 1日1回（JST 7:00、`workflow_dispatch`で手動実行も可） |
...
| B | 商品管理番号 |
| C | 商品名 |
| D | 販売価格（`variants.{variantId}.standardPrice`の最安値） |
| E | 在庫状況（「在庫あり」/「在庫切れ」の二値） |
| F | 取得日時（JST） |

### 動作の特徴・注意点

- **履歴は持たない**：実行のたびにシート全体を「今日時点の最新状態」で上書きする（過去分の推移は残らない）
- **取得失敗・0件時はシートを書き換えない**：API側の異常や認証切れで不完全なデータが取れた場合に、前日までの正しいデータを誤って消さないための安全策
- **1店舗でも失敗したら全店舗分を書き込まない**：片方の店舗だけ最新化して、失敗した店舗の前日データを消してしまう事故を防ぐため
- **在庫は「数」ではなく二値**：`items.search`のレスポンスには在庫数量は含まれない（返るのは出荷リードタイム等の設定のみ）。そのため`isItemStockout=true`で絞り込んだ別クエリを追加実行し、対象商品を「在庫切れ」、それ以外を「在庫あり」として記録している（＝店舗ごとに2回分のページング処理が走る）
- バリエーション（SKU）を持つ商品は、価格は最安値を採用
- 商品管理番号は店舗ごとに採番されるため、店舗をまたいで一意ではない（突き合わせる際は「店舗名＋商品管理番号」の組で扱う）

---

## 出品データ同期ツール（yahoo_listing_sync.py）

楽天版と対になる、Yahoo!ショッピング側の出品データ日次スナップショットツール。同じ専用スプレッドシート（`RAKUTEN_LISTING_SPREADSHEET_ID`）内に、楽天とは別タブで記録する。

### 対象API・保存先

| 項目 | 内容 |
|---|---|
| データソース | Yahoo!ショッピング商品リストAPI `myItemList`（`GET https://circus.shopping.yahooapis.jp/ShoppingWebService/V1/myItemList`） |
| 対象店舗 | **2店舗**（`YAHOO_SHOP_NAME_1/2` + `YAHOO_SELLER_ID_1/2`）。ただし2店舗とも同一Yahoo!ショッピングアカウントで管理されているため、OAuth連携（Client ID/Secret/リフレッシュトークン）は1組のみで、API呼び出し時に`seller_id`だけ切り替える |
| 認証方式 | OAuth 2.0（`Authorization: Bearer <アクセストークン>`）。アクセストークンはリフレッシュトークンから都度再発行 |
| **リフレッシュトークンのローテーション** | 使うたびに新しい値に更新される（有効期限は公開鍵登録済みで4週間、使うたびにリセット）。そのためGitHub Secretsではなく、スプレッドシートの非公開タブ「Yahoo_Config」（A列`refresh_token`、B列に値）に保存し、実行のたびに読み込み・書き戻しを行う |
| ページング | `start`（開始位置、デフォルト1）+ `results`（最大100件、デフォルト25） |
| 保存先・タブ名 | 「Yahoo_出品データ」（楽天と同じ専用スプレッドシート内、別タブ） |
| 実行頻度 | 1日1回（JST 7:30、`workflow_dispatch`で手動実行も可） |
...
| C | 商品名（`Name`） |
| D | 価格（`Price`） |
| E | 在庫数（`Quantity`。`stock=true`指定で取得。楽天のitems.searchと違い実数量が取れる） |
| F | 取得日時（JST） |

### 動作の特徴・注意点

- **履歴は持たない・取得失敗時は書き換えない・1店舗でも失敗したら全店舗分を書き込まない**：楽天版と同じ安全策
- **レスポンスはXML形式**（楽天はJSON）。標準ライブラリ`xml.etree.ElementTree`でパース
- **`query`または`stcat_key`のどちらかが必須パラメータ**：現状は`query=""`（空文字）で全件取得を試みている。これで全件ヒットしない場合は、SHPカテゴリ検索APIで全カテゴリキーを取得し、カテゴリ単位でページングする方式に変更する必要がある（実データでの検証待ち）
- レート制限は1クエリー/秒（楽天と同様、ページ間にsleepを入れている）
